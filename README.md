...
- Maintains per-user conversation state in a JSON store
...
- `STATE_DIR`: directory for persistent conversation storage. Defaults to `.data`.
- `STATE_FILE`: optional full JSON store file path. Leave blank unless you need a custom filename.
...
Then redeploy. The bot will write to `/var/data/conversations.json`, so leads and quotes remain available after restarts and deploys. Without a persistent disk, Render web service files can be lost on restart.

Production recommendations:

- For high volume, replace the file store with Redis or Postgres.
